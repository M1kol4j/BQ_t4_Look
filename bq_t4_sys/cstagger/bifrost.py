...
class OSC_data:
    def __init__(self, snap, template='qsmag-by00_t%03i', meshfile=None, fdir='.',
                 verbose=True,dtype='f4', big_endian=False, no_aux=False):
        ''' Main object for extracting Bifrost datacubes. '''

        self.snap     = snap
        self.fdir     = fdir
        # self.template = fdir+'/'+template % (snap)
        self.template = template % (snap)
        ...
        if self.boundarychk:
            print "[warning] Boundary cells will be included!"
            self.nz = self.nz + 2*self.params['mb']
        ...
        self.compvars = ['ux', 'uy', 'uz', 'ee', 's', 'bxc', 'byc', 'bzc', 'rup', 'dxdbup', 'dxdbdn', 'dydbup', 'dydbdn', 'dzdbup', 'dzdbdn'] # composite variables
    ...
    def read_params(self):
        ...
        # cubes may have extra cells for boundary

        try:
            self.boundarychk = self.params['boundarychk']
        except KeyError:
            raise KeyError('read_params: could not find boundarychk idl file!')
    ...
    def read_mesh(self, meshfile):
        ...
        assert mz == ( self.nz - (self.boundarychk * (2*self.params['mb'])) )

        self.z       = N.array([float(v) for v in f.readline().strip('\n').split()])
        self.zdn     = N.array([float(v) for v in f.readline().strip('\n').split()])
        self.dzidzup = N.array([float(v) for v in f.readline().strip('\n').split()])
        self.dzidzdn = N.array([float(v) for v in f.readline().strip('\n').split()])

        f.close()

        if self.boundarychk:
            
            # extend 'z', 'zdn', 'dzidzup' &  'dzidzdn' by boundary cells
            
            mb = self.params['mb']
            
            # z -> [ zbot, z, ztop ]
            zbot = [ self.z[0]-x*(self.z[1]-self.z[0]) for x in xrange(1,mb+1) ]
            ztop = [ self.z[mz-1]+x*(self.z[mz-1]-self.z[mz-2]) for x in xrange(1,mb+1)]
            self.z = N.concatenate((zbot,self.z,ztop))

            # zdn -> [ zbot, zdn, ztop ]
            zbot = [ self.zdn[0]-x*(self.zdn[1]-self.zdn[0]) for x in xrange(1,mb+1) ]
            ztop = [ self.zdn[mz-1]+x*(self.zdn[mz-1]-self.zdn[mz-2]) for x in xrange(1,mb+1)]
            self.zdn = N.concatenate((zbot,self.zdn,ztop))

            # dzidzup -> [ zbot, dzidzup, ztop ]
            zbot = N.zeros(mb) + self.dzidzup[ 0]
            ztop = N.zeros(mb) + self.dzidzup[-1]
            self.dzidzup = N.concatenate((zbot, self.dzidzup, ztop))

            # dzidzdn -> [ zbot, dzidzup, ztop ]
            zbot = N.zeros(mb) + self.dzidzdn[ 0]
            ztop = N.zeros(mb) + self.dzidzdn[-1]
            self.dzidzdn = N.concatenate((zbot, self.dzidzdn, ztop))
    ...
    def getcompvar(self,var,slice=None):
        ...
        import cstagger.cstagger as cs
        ...
        if   var == 'ux':  # x velocity
            ...
            if not hasattr(self,'bxc'): self.bxc=self.variables['bxc']=self.getvar('bx',order='C')
            return cs.xup(self.bxc)

        elif var == 'byc':  # y field
            if not hasattr(self,'byc'): self.byc=self.variables['byc']=self.getvar('by',order='C')
            return cs.yup(self.byc)

        elif var == 'bzc':  # z field
            if not hasattr(self,'bzc'): self.bzc=self.variables['bzc']=self.getvar('bz',order='C')
            return cs.zup(self.bzc)

        elif var == 'dxdbup':  # z field
            if not hasattr(self,'bxc'): self.bxc=self.variables['bxc']=self.getvar('bx',order='C')
            return cs.ddxup(self.bxc)

        elif var == 'dydbup':  # z field
            if not hasattr(self,'byc'): self.byc=self.variables['byc']=self.getvar('by',order='C')
            return cs.ddyup(self.byc)

        elif var == 'dzdbup':  # z field
            if not hasattr(self,'bzc'): self.bzc=self.variables['bzc']=self.getvar('bz',order='C')
            return cs.ddzup(self.bzc)

        elif var == 'dxdbdn':  # z field
            if not hasattr(self,'bxc'): self.bxc=self.variables['bxc']=self.getvar('bx',order='C')
            return cs.ddxdn(self.bxc)

        elif var == 'dydbdn':  # z field
            if not hasattr(self,'byc'): self.byc=self.variables['byc']=self.getvar('by',order='C')
            return cs.ddydn(self.byc)

        elif var == 'dzdbdn':  # z field
            if not hasattr(self,'bzc'): self.bzc=self.variables['bzc']=self.getvar('bz',order='C')
            return cs.ddzdn(self.bzc)

        elif var == 'rup':
            return cs.xup(self.r)
